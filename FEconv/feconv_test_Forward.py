...
if __name__ == "__main__":
    ...
    if randU:
        U = torch.rand((batchsize,18,resolution,resolution,resolution),dtype = torchtype,device=device,requires_grad=True)
    else:
        U = torch.ones((batchsize,18,resolution,resolution,resolution),dtype = torchtype,device=device,requires_grad=True)
    if randrho:
